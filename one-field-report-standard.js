...
async function reportImage(r){
  if(r?.correctedStampedImage)return r.correctedStampedImage;
  if(r?.correctedImage){
    try{if(typeof Watermark!=='undefined'&&Watermark?.stamp)return await Watermark.stamp(r.correctedImage,r)}catch(_){}
    return r.correctedImage;
  }
...
async function make(){
  ...
  for(let i=0;i<data.length;i++){
    ...
    /* Marco corregido tiene prioridad visual y se vuelve a sellar para el reporte. */
    const imageData=await reportImage(r);
