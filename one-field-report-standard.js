/* ONE SHOP · REPORTE MODELO CAMPO v7 */
(()=>{
'use strict';
if(window.ONE_FIELD_REPORT_STANDARD)return;
const BUILD='one-field-report-standard-v7';
const safe=v=>String(v??'').trim();
const norm=v=>safe(v).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase().replace(/[^A-Z0-9]+/g,' ').replace(/\s+/g,' ').trim();
const pending=v=>!safe(v)||/pendiente|no disponible|sin dato/i.test(safe(v));
...
function territory(r){
 let region=clean(r.department||r.region),province=clean(r.province),district=clean(r.district),city=clean(r.city);
 if(!region&&norm(province)==='LIMA')region='Lima';
 if(!province){if(norm(region)==='LIMA'&&district)province='Lima';else if(norm(region)==='CALLAO'&&district)province='Callao';else if(city)province=city}
 if(!district&&city&&norm(city)!==norm(province))district=city;
 return{region:region||'PENDIENTE',province:province||'PENDIENTE',district:district||'PENDIENTE'};
}
...
async function make(){
 ...
 const wb=new ExcelJS.Workbook();wb.creator='ONE SHOP · ONPE-SGVC';wb.created=new Date();wb.subject='Reporte de evidencias de campo';wb.description='Evidencias de campo normalizadas a 4:3 con datos territoriales saneados.';
 ...
 ws.columns=[
  ...
  {header:'Provincia',key:'provincia',width:16},{header:'Distrito',key:'distrito',width:18},{header:'Dirección',key:'direccion',width:29},{header:'Alcalde',key:'alcalde',width:31},
 ...
 for(let i=0;i<data.length;i++){
   ...
   ws.addRow({
     id:i+1,fecha:clean(r.fecha),hora:clean(r.hora),ubicacion:locationText,empresa:provider(r),nota:party(r),latlon,
     clima:weather(r),altitud:Number.isFinite(alt)?`${Math.round(alt)} m`:'-',zona:firstLabel(r,['teamSector','zone','missionName'],'-'),
     region:t.region,provincia:t.province,distrito:t.district,direccion:administrativeAddress(r),alcalde:mayor(r),
     tipo:firstLabel(r,['type','evidenceType'],'PENDIENTE'),proceso:firstLabel(r,['electionProcess','process'],'')
   });
   ...
   if(/^data:image\//i.test(imageData))try{const ext=/^data:image\/png/i.test(imageData)?'png':'jpeg';const imgId=wb.addImage({base64:imageData.split(',')[1],extension:ext});ws.addImage(imgId,{tl:{col:1.02,row:rowNo-0.88},ext:{width:128,height:96},editAs:'oneCell'})}catch(_){}
 }
 ws.autoFilter={from:{row:1,column:1},to:{row:Math.max(1,data.length+1),column:18}};ws.getColumn(1).alignment={vertical:'middle',horizontal:'center'};ws.getColumn(2).alignment={vertical:'middle',horizontal:'center'};
 ws.pageSetup={orientation:'landscape',fitToPage:true,fitToWidth:1,fitToHeight:0,paperSize:9,margins:{left:.2,right:.2,top:.35,bottom:.35,header:.15,footer:.15}};

 const tech=wb.addWorksheet('DATOS_TECNICOS',{views:[{state:'frozen',ySplit:1}]});
 tech.columns=[
  {header:'ID',key:'id',width:6},{header:'CÓDIGO',key:'code',width:24},{header:'VERIFICADOR',key:'verify',width:18},{header:'FECHA',key:'fecha',width:13},{header:'HORA',key:'hora',width:12},
  {header:'LATITUD',key:'lat',width:16},{header:'LONGITUD',key:'lon',width:16},{header:'GPS ±m',key:'accuracy',width:12},{header:'ALTITUD',key:'altitude',width:11},{header:'UBIGEO',key:'ubigeo',width:12},
  {header:'REGIÓN',key:'region',width:16},{header:'PROVINCIA',key:'province',width:16},{header:'DISTRITO',key:'district',width:18},{header:'DIRECCIÓN CAPTURA',key:'address',width:40},{header:'ALCALDE',key:'mayor',width:31},
  {header:'TIPO',key:'type',width:16},{header:'PARTIDO',key:'party',width:28},{header:'EMPRESA / PROVEEDOR',key:'provider',width:28},{header:'PROCESO',key:'process',width:12},
  {header:'RESOLUCIÓN ORIGINAL',key:'originalSize',width:20},{header:'RESOLUCIÓN NORMALIZADA',key:'normalizedSize',width:22},{header:'NORMALIZACIÓN',key:'normalization',width:15},{header:'SHA-256 ORIGINAL',key:'sourceHash',width:68},{header:'SHA-256 MARCADA',key:'stampedHash',width:68},{header:'MAPA',key:'map',width:17}
 ];
 header(tech);
 data.forEach((r,i)=>{const g=r.gps||{},t=territory(r),url=maps(r),rn=i+2;tech.addRow({id:i+1,code:clean(r.photoCode),verify:clean(r.verifyCode),fecha:clean(r.fecha),hora:clean(r.hora),lat:Number.isFinite(Number(g.latitude))?Number(g.latitude):'',lon:Number.isFinite(Number(g.longitude))?Number(g.longitude):'',accuracy:Number.isFinite(Number(r.accuracy??g.accuracy))?Math.round(Number(r.accuracy??g.accuracy)):'',altitude:Number.isFinite(Number(r.altitude??g.altitude))?Math.round(Number(r.altitude??g.altitude)):'',ubigeo:/^\d{6}$/.test(safe(r.ubigeo))?safe(r.ubigeo):'',region:t.region,province:t.province,district:t.district,address:captureAddress(r),mayor:mayor(r),type:firstLabel(r,['type','evidenceType'],'PENDIENTE'),party:party(r),provider:provider(r),process:firstLabel(r,['electionProcess','process']),originalSize:r.sourceWidth&&r.sourceHeight?`${r.sourceWidth} × ${r.sourceHeight}`:'',normalizedSize:r.normalizedWidth&&r.normalizedHeight?`${r.normalizedWidth} × ${r.normalizedHeight}`:(r.reportImage4x3?'1600 × 1200':''),normalization:r.captureNormalization||r.reportImageVersion||'',sourceHash:clean(r.sourceHash),stampedHash:clean(r.stampedHash),map:url?'Abrir ubicación':''});tech.getRow(rn).alignment={vertical:'middle',wrapText:true};if(i%2===0)tech.getRow(rn).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFF7FAFD'}};link(tech.getCell(rn,25),url?'📍 Abrir ubicación':'Sin ubicación',url)});
 tech.autoFilter={from:{row:1,column:1},to:{row:Math.max(1,data.length+1),column:25}};
 ...
 const info=wb.addWorksheet('INFORMACIÓN');info.getCell('A1').value='ONE SHOP · ONPE-SGVC · REPORTE DE EVIDENCIAS';info.getCell('A1').font={bold:true,size:15,color:{argb:'FF0B2D5B'}};info.getCell('A3').value='Criterio de foto';info.getCell('B3').value='La captura conserva su original y genera una copia normalizada 4:3 de 1600×1200. En Excel todas las evidencias se insertan a 128×96 px, centradas y sin deformarse.';info.getCell('A4').value='Territorio';info.getCell('B4').value='Región, Provincia, Distrito y UBIGEO se normalizan antes del reporte. Los valores internos numéricos no se exportan como etiquetas.';info.getCell('A5').value='Empresa';info.getCell('B5').value='Proveedor identificado únicamente cuando la evidencia es PANEL.';info.getCell('A6').value='Nota';info.getCell('B6').value='Organización política / partido.';info.getColumn(1).width=24;info.getColumn(2).width=105;
...
function patch(){try{if(typeof Reports==='undefined')return false;Reports.__territorialStandardV7=true;Reports.__territorialStandard=true;Reports.makeExcel=make;Reports.invalidate?.();return true}catch(_){return false}}
let n=0;const t=setInterval(()=>{if(patch()||++n>100)clearInterval(t)},50);patch();window.ONE_FIELD_REPORT_STANDARD={BUILD,make,patch,territory,provider,party};
})();