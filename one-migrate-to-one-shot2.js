...
function toast(m,ms=3000){try{if(typeof UI!=='undefined'&&UI.toast)UI.toast(m,ms,{placement:'top',tone:'soft'})}catch(_){}}
...
function open(){build();$('oneShopUpgradeModal').classList.add('open');const total=state()?.records?.length||0;$('oneShopUpgradeProgress').textContent=`Este dispositivo tiene ${total} evidencias locales. ${lastResult?`Último respaldo: ${lastResult.ok} OK · ${lastResult.err} error.`:'Aún no se ejecutó la preparación.'}`}
