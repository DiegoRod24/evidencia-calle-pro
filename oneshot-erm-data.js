...
(()=>{
'use strict';
const BUILD='one-shop-field-boot-20260829-01-fastcapture';
...
function patchFastCapture(){
  try{
    ...
    const rearIndex=()=>{let best=-1,score=999;(State.devices||[]).forEach((d,i)=>{const l=String(d.label||'');const s=frontRx.test(l)?99:rearRx.test(l)?0:/ultra|0\.5|0,5|gran angular/i.test(l)?3:5;if(s<score){score=s;best=i}});return best;};
    Camera.enumerate=async()=>{await baseEnumerate();const current=State.currentTrack?.getSettings?.().deviceId||'';if(current){const idx=State.devices.findIndex(d=>d.deviceId===current);if(idx>=0)State.deviceIndex=idx;}else{const idx=rearIndex();if(idx>=0)State.deviceIndex=idx;}return State.devices;};
    Camera.attempts=()=>{const q=Camera.qualityConstraints(),list=[],idx=rearIndex(),d=idx>=0?State.devices[idx]:null;if(d?.deviceId&&!frontRx.test(String(d.label||'')))list.push({audio:false,video:{deviceId:{exact:d.deviceId},...q}});list.push({audio:false,video:{facingMode:{exact:'environment'},...q}});list.push({audio:false,video:{facingMode:{ideal:'environment'},...q}});list.push({audio:false,video:{facingMode:'environment'}});list.push({audio:false,video:true});return list;};
    const baseStart=Camera.start.bind(Camera);let correcting=false;
    Camera.start=async(opts={})=>{State.cameraFacing='back';const ok=await baseStart(opts);if(!ok)return ok;try{const facing=Camera.detectFacing?.()||'back';State.cameraFacing=facing;if(facing==='front'&&!correcting){await Camera.enumerate();const idx=rearIndex(),target=idx>=0?State.devices[idx]:null,current=State.currentTrack?.getSettings?.().deviceId||'';if(target?.deviceId&&target.deviceId!==current&&!frontRx.test(String(target.label||''))){correcting=true;State.deviceIndex=idx;const fixed=await baseStart({silent:true,force:true});correcting=false;if(fixed){State.cameraFacing=Camera.detectFacing?.()||'back';return fixed;}}}}catch(_){correcting=false;}return ok;};
    ...
    setTimeout(prime,80);window.addEventListener('pageshow',()=>setTimeout(prime,90));document.addEventListener('visibilitychange',()=>{if(!document.hidden)setTimeout(prime,80);});
    return true;
...
async function boot(){
  ...
  try{
    await load('/one-shop-stable-runtime.js?v=20260829-01-fastcapture','ONE_SHOP_STABLE_RUNTIME');
    await load('/one-shop-field-quality-v578.js?v=20260829-01-fastcapture','ONE_SHOP_FIELD_QUALITY_V578');
    await load('/one-shop-excel-thumb-v579.js?v=20260829-01-fastcapture','ONE_SHOP_EXCEL_THUMB_V579');
    await load('/one-shop-storage-lite-v578.js?v=20260829-01-fastcapture','ONE_SHOP_STORAGE_LITE_V578');
    await load('/one-field-report-standard.js?v=20260829-01-fastcapture','ONE_FIELD_REPORT_STANDARD_V8');
    patchFastCapture();
