...
function excel(){return loadScript('https://cdn.jsdelivr.net/npm/exceljs@4.4.0/dist/exceljs.min.js','ExcelJS')}
...
function patchBoot(){
  ...
  try{if(typeof Places!=='undefined'&&Places.promptRelation){const base=Places.promptRelation.bind(Places);Places.__stablePrompt=base;Places.promptRelation=()=>null}}catch(_){}
  try{if(typeof State!=='undefined'&&State.settings){if(localStorage.getItem('oneShopStablePrefs570')!=='1'){State.settings.nearbyHistoryEnabled=false;localStorage.setItem('oneShopStablePrefs570','1');try{Store?.saveLite?.()}catch(_){}}}}catch(_){}
...
function boot(){patchBoot();styleEvidence();injectOptionalCloud();setTimeout(()=>{patchBoot();injectOptionalCloud()},500);setTimeout(()=>injectOptionalCloud(),1800)}
