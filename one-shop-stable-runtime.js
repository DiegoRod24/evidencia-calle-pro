/* ONE SHOP v5.7.4 · CAMPO ESTABLE + REPORTE TERRITORIAL */
(()=>{
'use strict';
if(window.ONE_SHOP_STABLE_RUNTIME)return;
const BUILD='one-shop-v5.7.4-field-essential-01';
...
document.addEventListener('click',e=>{
  ...
  const id=el.id||'',txt=((el.textContent||'')+' '+id+' '+(el.dataset?.view||'')+' '+(el.dataset?.nav||'')).toLowerCase();
  if(/evidencia|editar|lugares|territorio|tramo|cartel|pancarta/.test(txt))fieldTools().then(()=>{try{if(/lugares|territorio|tramo|cartel|pancarta/.test(txt))window.ONE_SHOP_FIELD_TOOLS?.injectTramo?.();if(/editar/.test(txt)&&document.getElementById('editModal')?.classList.contains('open'))window.ONE_SHOP_FIELD_TOOLS?.injectEditor?.()}catch(_){}}).catch(()=>{});
  let dep=null,label='';
  if(excelIds.has(id)||/excel|xlsx/.test(txt)){dep=excel();label='Excel'}
  else if(mapIds.has(id)||/cobertura territorial|mapa real|dibujar sector/.test(txt)){dep=leaflet();label='mapa'}
  if(!dep)return;
...
function patchBoot(){
  ...
  try{if(typeof State!=='undefined'&&State.settings){if(localStorage.getItem('oneShopStablePrefs574')!=='1'||State.settings.nearbyHistoryEnabled!==false){State.settings.nearbyHistoryEnabled=false;localStorage.setItem('oneShopStablePrefs574','1');try{Store?.saveLite?.()}catch(_){}}}}catch(_){}
...
function styleEvidence(){if($('oneShopStableCss'))return;const s=document.createElement('style');s.id='oneShopStableCss';s.textContent=`#evidenceList .eMedia{background:#071326!important;overflow:hidden!important}#evidenceList .eMedia img,.reportPreviewList img{object-fit:contain!important;object-position:center!important;background:#071326!important}#oneShopOptionalCloud small{color:inherit;opacity:.72}`;document.head.appendChild(s)}
...
window.ONE_SHOP_STABLE_RUNTIME={BUILD,excel,leaflet,fieldTools,patchBoot};
})();
